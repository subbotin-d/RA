...
fun Order.getTotalAmount(): Double = this.products.sumByDouble { product -> product.price }
                                    //this.products.fold( 0.0, { sum, product -> sum + product.price })
...
fun Shop.getCustomersWithAtLeastOneDeliveredOrder(): List<Customer> =
        this.customers.filter { customer -> customer.orders.any { order -> order.isDelivered }}
...
fun Shop.getCustomersWithTotalAmountMoreThan(amount: Double): List<Customer> =
        this.customers.filter { customer ->
            customer.orders.flatMap { order -> order.products }.sumByDouble { product -> product.price } >= amount }
...
fun Shop.getMostPopularProduct(): Product? = this.customers
        .flatMap { customer -> customer.orders }
        .flatMap { order -> order.products }
        .fold<Product, MutableMap<Product,Int>>(
            mutableMapOf<Product, Int>() ,
            { productsRegistry, product ->
                productsRegistry[product] = productsRegistry.getOrDefault(product, 0).plus(1)
                productsRegistry
            })
        .maxBy { registry -> registry.value }
        ?.key