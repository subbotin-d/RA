...
fun Shop.getCustomersWithTotalAmountMoreThan(amount: Double): List<Customer> = this.customers.filter { customer -> customer.orders
    .flatMap { order -> order.products }
    .sumByDouble { product -> product.price } >= amount}
...
fun Shop.getMostPopularProduct(): Product? = customers.flatMap { customer -> customer.orders
    .flatMap { order -> order.products } }
    .groupingBy { it }
    .eachCount()
    .maxBy { it.value }?.key